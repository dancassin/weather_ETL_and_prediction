...
def transform_weather_df(df):
    df.weather = pd.json_normalize(df["weather"][0][0])["description"][
        0
    ]  # gets short description from sub dictionary
    columns_to_keep = df[
        [
            "weather",
            "visibility",
    ...
    columns_to_keep.rename(
        columns={
            ...
            "wind.deg": "wind_deg",
            "clouds.all": "cloudiness_pct",
            "sys.sunrise": "dt_sunrise",
            "sys.sunset": "dt_sunset",
            "t10": "10_centi_soil_depth_temp",
    ...
    columns_to_convert = [
        ...
        "temp_min",
        "temp_max",
        "10_centi_soil_depth_temp",
        "soil_surface_temp",
    ]

    columns_to_keep[columns_to_convert] = columns_to_keep[columns_to_convert].apply(
        kelvin_to_farenheit_calc
    )  # converting kelvin to farenheit

    columns_to_keep["date"] = datetime.fromtimestamp(columns_to_keep["dt"]).strftime(
        "%Y-%m-%d"
    )
    columns_to_keep["dt_sunrise"] = datetime.fromtimestamp(
        columns_to_keep["dt_sunrise"]
    ).strftime("%H:%M:%S")
    columns_to_keep["dt_sunset"] = datetime.fromtimestamp(
        columns_to_keep["dt_sunset"]
    ).strftime("%H:%M:%S")

    columns_to_keep["date"] = columns_to_keep["date"].apply(
